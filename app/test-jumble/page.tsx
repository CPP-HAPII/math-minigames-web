...
/**
 * /test-jumble — temporary harness to play a real Jumble question end-to-end.
...
export default function TestJumblePage() {
  ...
  const question = jumbleBank[index];

  // Log the accepted answers so the tester knows what to build.
  useEffect(() => {
    if (question) {
      console.log(
        `[test-jumble] question ${index + 1}/${jumbleBank.length} · id=${question.id}\n` +
          `  options: ${JSON.stringify(question.optionList)}\n` +
          `  accepted: ${JSON.stringify(question.multiAcceptedAnswers)}`,
      );
    }
  }, [question, index, jumbleBank.length]);
  ...
  if (!question) {
    return <main style={{ fontFamily: 'monospace', padding: '2rem' }}>No Jumble questions found in the bank.</main>;
  }
  ...
  return (
    <main>
      <div style={{ fontFamily: 'monospace', padding: '0.75rem 1rem', display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', borderBottom: '1px solid #ccc' }}>
        <span>
          test-jumble · question {index + 1}/{jumbleBank.length} · id=<strong>{question.id}</strong> · assist=<strong>{assistLevel}</strong>
        </span>
        <button
          onClick={() => setIndex((i) => (i + 1) % jumbleBank.length)}
      ...
      <JumbleGame
        ...
        onComplete={(wasCorrect) =>
          console.log('[test-jumble] onComplete →', { id: question.id, wasCorrect })
        }
      />
    </main>
  );
